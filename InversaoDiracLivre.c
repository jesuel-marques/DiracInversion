...
double U[Nt][Nxyz][Nxyz][Nxyz][d][3][3][2];	//	Matriz de elos, utilizados para calcular operador de Dirac
...
int main(){
	...
	sprintf(nomearquivoOpDirac,"./DiracOperators/OpDiracFree_Nxyz_%d_Nt_%d_kappa_%.6lf.txt",Nxyz,Nt,-menoskappa[RE]);
	...
	sprintf(nomearquivoinverso,"./DiracInverses/InverseFree_Nxyz_%d_Nt_%d_kappa_%.6lf.txt",Nxyz,Nt,-menoskappa[RE]);

	ImprimirInverso(nomearquivoinverso); /* Imprime Inverso do Operador de Dirac para testes (Comparação com Mathematica) */


	for(int inddirac=0;inddirac<d;inddirac++)
		for(int indcor=0;indcor<3;indcor++){
			ProdutoDInversao(Inverso[inddirac][indcor],Delta[inddirac][indcor]);
		}

	sprintf(nomearquivodelta,"./Deltas/DeltaFree_Nxyz_%d_Nt_%d_kappa_%.6lf.txt",Nxyz,Nt,-menoskappa[RE]);
	ImprimirDelta(nomearquivodelta,Delta);

	sprintf(nomearquivoMZ,"./MZ/MZFree_Nxyz_%d_Nt_%d_kappa_%.6lf.txt",Nxyz,Nt,-menoskappa[RE]);
